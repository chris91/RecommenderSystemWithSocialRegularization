...
class TestIndividualBasedRecommender(unittest.TestCase):
    ...
    def test_individual_based_social_regularization(self):
        reader = Reader(rating_scale=(1, 5))
        data = Dataset.load_from_df(self.ratings, reader)

        trainset, testset = train_test_split(data, test_size=.25)

        algoIBSR = IndividualBasedSocialRegularization(user_max=10)
        algoIBSR.fit(trainset)

        predictions = algoIBSR.test(testset)
    ...
    def test_with_cross_validation(self):
        ...
        algo = IndividualBasedSocialRegularization(user_max=10)
